...
-- O objetivo desse trabalho é definir uma função que, dado um documento, gera um índice das palavras que ocorrem nesse documento. O programa deve ter como entrada um arquivo texto.
...
-- a) Separar o documento em linhas:
lines :: Doc -> [Line]
...
-- f) Eliminar, da lista de números de páginas em que cada palavra ocorre, as repetições de um mesmo número de linha:
...
-- Antes de separar cada linha do seu texto em palavras, devem ser eliminados da linha os caracteres de pontuação, os quais não devem ser incluídos no índice (olhar as funções isAlpha, isSpace, isDigit, zip).
